        ...
        if subb == '':
            ...
            def enter_data_DB():
                ...
                def create_csv():
                    global subb
                    subb = SUB_ENTRY.get()
                    csv_name = 'C:/Users/rahul/Downloads/Attendace_management_system/Attendance/Manually Attendance/' + subb + '.csv'
            ...
            def attf():
                import subprocess
                subprocess.Popen(
                    r'explorer /select,"C:\Users\rahul\Downloads\Attendace_management_system\Attendance\Manually Attendance\-------Check atttendance-------"')
...
def subjectchoose():
        ...
        if time.time() < future:
            if sub == '':
                ...
                while True:
                    ...
                    for (x, y, w, h) in faces:
                        ...
                        if conf < 200:
                            ...
                            # Check if this student has an entry within the last 5 minutes
                            if aa in previous_time and (ts - previous_time[aa]) < 300:
                                continue  # Skip this entry if it's within 5 minutes of the last entry
                ...
                if os.path.isfile(fileName):
                    # If the file already exists, append the attendance data
                    existing_attendance = pd.read_csv(fileName)
                    combined_attendance = pd.concat([existing_attendance, attendance], ignore_index=True)
                    # Calculate time spent in class for each student
                    combined_attendance['Time Spent in Class'] = combined_attendance.groupby('Name')['Time'].transform(
                        lambda x: round((pd.to_datetime(x) - pd.to_datetime(x.min())).dt.total_seconds() / 60,
                                        2) if len(x) > 1 else 0)
                    combined_attendance.to_csv(fileName, index=False)
                else:
                    # If the file doesn't exist, create it and write the attendance data
                ...
                # Open the attendance file and calculate time spent in class for the same subject on the same date
                cs = f'C:/Users/rahul/Downloads/Attendace_management_system/{fileName}'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    next(reader)  # Skip the header row
                    previous_time = None
                    for row in reader:
                        current_time = datetime.datetime.strptime(row[3], "%H:%M:%S")
                        if previous_time:
                            time_diff = current_time - previous_time
                            minutes_present = round(time_diff.total_seconds() / 60, 2)
                            row.append(f"{minutes_present} minutes present")
    ...
    def Attf():
        os.startfile(r'C:\Users\rahul\Downloads\Attendace_management_system\Attendance')
    ...
    fill_a = tk.Button(windo, text="Fill Attendance", fg="white", command=Fillattendances, bg="SkyBlue1", width=20,
                       height=2,
                       activebackground="white", font=('times', 15, ' bold '))
    ...
    def log_in():
        ...
        if username == 'Rahul':
            if password == 'Rahul':
                ...
                cs = 'C:/Users/rahul/Downloads/Attendace_management_system/StudentDetails/StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
...
quitWindow = tk.Button(window, text="Check Registered students", command=admin_panel, fg="black",
                       bg="SkyBlue1", width=20, height=3, activebackground="white", font=('times', 15, ' bold '))
